...
def main():
    ...
    if ENUMERATE_CELLS:
        for idx, row in grid.iterrows():
            ...
            ax.text(x, y, str(idx + 1), ha="center", va="center", fontsize=9, color="darkgreen")

    if not KEEP_ASPECT_RATIO:
        ax.set_aspect("auto")

    ax.set_facecolor("lightskyblue")
    plt.subplots_adjust(top=0.945, bottom=0.06, left=0.055, right=0.96, hspace=0.2, wspace=0.2)
    plt.title(f"Europe")
    plt.show()
